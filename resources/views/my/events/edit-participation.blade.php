...
@section('title', 'Редактировать заявку на участие в мероприятии')

@section('h1', 'Редактировать заявку на участие в мероприятии')
...
@section('content')
    ...
    <form class="registration__form form" @submit.prevent="submit" x-data="{
        form: $form('post', '{{ route('participation.edit', $conference->slug) }}', {
            ...
            phone: '{{ $participation->phone?->raw() }}',
			affiliations: affiliations,
            email: '{{ $participation->email }}',
            orcid_id: '{{ $participation->orcid_id }}',
            website: '{{ $participation->website }}',
			participation_type: '{{ $participation->participation_type }}',
			is_young: {{ $participation->is_young ? 'true' : 'false' }},
        }),
