					...
					<div class="tw-flex tw-gap-3">
						...
						<template x-for="section in conference.sections">
							<button class="button" 
								...
								@click="filterSection(section.id)"
							></button>
						</template>
					</div>
				</template>
			</div>
			{{-- <button class="button button_outline">CSV</button> --}}
			<button @click="getCsv">
		...
		document.addEventListener('alpine:init', () => {
			Alpine.data('theses', () => ({
				...
				filterSection(id) {
					...
					if (id === '*') {
						this.filtered = this.conference.theses
						return
					}

					this.filtered = this.conference.theses.filter(el => {
						return id == el.section_id
					})
				},
				getCsv() {
					let thesesIds = []
					...
					axios
						...
						.then(response => saveAs(response.data, `abstracts-${section}.csv`))
